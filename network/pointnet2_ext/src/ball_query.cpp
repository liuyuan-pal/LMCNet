...
at::Tensor ball_query_feats(at::Tensor new_xyz, at::Tensor xyz, const float radius,
                            const int nsample) {
  CHECK_CONTIGUOUS(new_xyz);
  CHECK_CONTIGUOUS(xyz);
  CHECK_IS_FLOAT(new_xyz);
  ...
  if (new_xyz.type().is_cuda()) {
    query_ball_feats_kernel_wrapper(xyz.size(0), xyz.size(1), new_xyz.size(1), xyz.size(2),
                                    radius, nsample, new_xyz.data<float>(),
                                    xyz.data<float>(), idx.data<int>());
  } else {
    AT_CHECK(false, "CPU not supported");
